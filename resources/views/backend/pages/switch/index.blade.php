...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>Relatório Switchs/Roteadores</h1>
                </div>
                <div class="col-sm-6">
    ...
    <div class="grind-actions">
        <div class="col-sm-6">
            <form action="{{ route('admin.search.router') }}" method="POST">
                @csrf
                @foreach($router as $route)
                <input name="id" value="{{ $route->id }}" hidden>
                @endforeach
    ...
    <div class="container-computer-table">
        <table class="table">
            ...
            <tbody>
                @foreach($router as $route)
                <tr>
                    <td>{{ $route->fabricate }}</td>
                    <td>{{ $route->serial }}</td>
                    <td>{{ $route->qtdPortas }}</td>
                    <td>{{ $route->localizacao }}</td>
                    <td>
                        <div class="grind-actions-buttons">
                            <form action="{{ route('admin.route.printers') }}" method="POST">
                                @csrf
                                <input name="id" value="{{ $route->id }}" hidden>
                            ...
                            <form action="{{ route('admin.route.download') }}" method="POST">
                                @csrf
                                <input name="id" value="{{ $route->id }}" hidden>
                                <button type="submit" id="btn-custom" class="btn btn-warning bi bi-box-arrow-down">
                                </button>
                            </form>
                            <form action="{{ route('admin.route.delete') }}" method="POST">
                                @csrf
                                <input name="id" value="{{ $route->id }}" hidden>
